...
type User struct {
	Id       string `json:"id"` // tag json digunakan untuk menentukan nama field yang akan di tampilkan di response
...
// insert data user
func StoreUser(id string, email string, username string, password string) (Response, error) {
	var res Response
	...
	sqlStatement := "INSERT INTO `users`(`id`, `email`, `username`, `password`) VALUES (?,?,?,?)"
	...
	result, err := stmt.Exec(id, email, username, password)
...
func UpdateUser(id string, email string, username string, password string) (Response, error) {
	var res Response
	...
	result, err := stmt.Exec(email, username, password, id)
...
func CheckLogin(email, password string) (bool, error) {
	...
	err := con.QueryRow(sqlStatement, email).Scan(
		&obj.Id, &obj.Email, &obj.Username, &pwd,
	)
